...
class ResNetBackboneNet(nn.Module):
    ...
    def forward(self, x): # x.shape [32, 3, 256, 256]
        if self.freeze:
            with torch.no_grad():
                x = self.conv1(x)   # x.shape [32, 64, 240, 320]
                x = self.bn1(x)
                x = self.relu(x)
                #x_low_feature = self.maxpool(x) # x.shape [32, 64, 240, 320]
                x_low_feature=x
                x = self.layer1(x_low_feature)  # x.shape [32, 256, 240, 320]
                x = self.layer2(x)  # x.shape [32, 512, 120, 160]
                x = self.layer3(x)  # x.shape [32, 1024, 60, 80]
                x_high_feature = self.layer4(x)  # x.shape [32, 2048, 30, 40]
